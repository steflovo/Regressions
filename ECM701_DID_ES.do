...
coefplot  ev1,  keep(id_treated_*) ///
		yaxis("") xline(3.5) yline(0) vertical  name(method1, replace)  ///
		msymbol(circle) mcolor(blue) xtitle("Time") ///
		xlabel (1 "-3"  2 "-2"  3 "-1"  4 "0"  5 "1"  6 "2"  7 "3"  ) ///
		ytitle(Coefficients) legend(off) ciopts(color(black)) p5(msymbol(lgx)) 
